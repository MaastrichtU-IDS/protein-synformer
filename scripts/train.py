...
@click.option("--folds", type=int, default=1)
def main(
    config_path: str,
    seed: int,
    debug: bool,
    ...
    for fold_idx in range(folds):
        fold_exp_name = f"{exp_name}_fold{fold_idx}"
        config.train.exp_name = fold_exp_name

        # Initialize WandB logging if enabled
        run = None
        if os.path.exists("configs/wandb.yml"):
        ...
        if OmegaConf.select(config, "model.decoder.lora"):
            ...
            print(n_params(model.model.decoder.lora_dec, only_trainable=True), "\t" + "Trainable parameters: lora_dec")
        ...
        trainer = pl.Trainer(
            ...
            callbacks=[
                callbacks.ModelCheckpoint(
                    dirpath=os.path.join(log_dir, fold_exp_name, exp_ver, "checkpoints"),
                    save_last=True,
                    monitor="val/loss",
                    mode="min",
                    save_top_k=5,
            ...
            logger=[
                loggers.TensorBoardLogger(log_dir, name=fold_exp_name, version=exp_ver),
            ],
